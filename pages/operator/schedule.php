...
function getShows($db, $today, $status)
{
    $stmt = $db->prepare(
        "SELECT s.*, f.title, f.duration,
         ...
         WHERE s.show_date = ? AND s.status = ?
         ORDER BY s.show_time"
    );
    $stmt->execute([$today, $status]);
    return $stmt->fetchAll();
}
 
$scheduled = getShows($db, $today, 'scheduled');
$ready     = getShows($db, $today, 'ready');
$showing   = getShows($db, $today, 'showing');
$finished  = getShows($db, $today, 'finished');
...
.dot-scheduled { background: #F9A825; }
.dot-ready     { background: #1565C0; }
.dot-showing   { background: #C62828; }
...
.badge-scheduled { background: #FFF9C4; color: #F57F17; }
.badge-ready     { background: #BBDEFB; color: #0D47A1; }
.badge-showing   { background: #FFEBEE; color: #C62828; }
.badge-finished  { background: #F5F5F5; color: #757575; }
...
.btn-green:hover { background: #f1f8f1; }
    ...
    <div class="content">
        ...
        <div class="viewer-card">
            <div class="viewer-left">
                <div class="viewer-label">Total Penonton Hari Ini</div>
                <div class="viewer-num"><?= number_format($totalViewers) ?></div>
                <div class="viewer-sub"><?= date('d F Y') ?></div>
            </div>
