...
if ($search && $filterTab === 'fnb') {
    $fnbWhere[]  = '(u.name LIKE ? OR of2.order_id LIKE ? OR fm.name LIKE ?)';
    $fnbParams[] = "%$search%";
    $fnbParams[] = "%$search%";
    $fnbParams[] = "%$search%";
}
...
<head>
  ...
  <style>
    ...
    .tab-btn.active.fnb-tab { background: var(--orange); }
    ...
    .search-bar select:focus { border-color: var(--red); }
    .search-bar select option { background: #1a1d27; }
    ...
    .btn-orange:hover { background: #ea6c0c; }
    ...
    .cat-popcorn { background:rgba(249,115,22,0.15); color:#f97316; }
    .cat-drinks  { background:rgba(59,130,246,0.15);  color:#3b82f6; }
    .cat-snacks  { background:rgba(168,85,247,0.15);  color:#a855f7; }
...
<div class="admin-layout">
  ...
  <div class="main-content">
    ...
    <div class="page-content">
              ...
              <tbody>
                ...
                ?>
                  <tr>
                    ...
                    <td>
                      <div style="font-weight:600"><?= htmlspecialchars($b['cname']) ?></div>
      ...
      <!-- Film Stats -->
      <div class="card">
        <div class="card-hdr"><h3>Statistik Per Film</h3></div>
        <table>
          <thead>
            <tr>
              <th>Film</th>
              ...
              <tbody>
                <?php foreach ($fnbOrders as $r): ?>
                  <tr>
                    ...
                    <td>
                      <?php if ($r['cname']): ?>
                        <div style="font-weight:600"><?= htmlspecialchars($r['cname']) ?></div>
                      <?php else: ?>
                        <div style="color:var(--text-muted);font-size:0.8rem">Guest</div>
                      <?php endif; ?>
                      <div style="font-size:0.75rem;color:var(--text-muted)">Via <?= strtoupper($r['payment_method']) ?></div>
                    </td>
      ...
      <!-- F&B Item Stats -->
      <div class="card">
        <div class="card-hdr"><h3>Statistik Penjualan Menu</h3></div>
        <?php if (empty($fnbStats)): ?>
          <div class="empty-state"><p>Belum ada data penjualan F&amp;B.</p></div>
        <?php else: ?>
          <table>
            <thead>
              <tr>
